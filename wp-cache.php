...
function check_up_on_preloading() {
	...
	if (
        ...
        && isset($value['t'])
        && ((time() - $value['t']) > 3600)
        && (false == wp_next_scheduled('wp_cache_preload_hook'))
    ) {
		if ( is_admin() ) {
...
/**
 * Get admin bar "Delete Cache" menu item configuration
* @return array
 */
function wpsc_get_admin_bar_item() {
    $node = [];
	if ( function_exists( 'current_user_can' ) && is_user_logged_in() ) {
            ...
            $node = array(
                ...
                'href' => wp_nonce_url(
                    add_query_arg(
                        [
                            'action' => 'delcachepage',
                            'path' => wp_supercache_get_uri_cache_dir($path),
                        ],
                        admin_url('index.php')
                    ),
                    'delete-cache'
        ...
        } elseif ( is_admin() && wpsupercache_site_admin() && current_user_can( 'manage_options' ) ) {
            $node = array(
                'parent' => false,
                'id' => 'delete-cache',
                'title' => __( 'Delete Cache', 'wp-super-cache' ),
