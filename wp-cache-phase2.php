...
function wp_cache_get_cookies_values() {
	...
	$string .= 'ble_show_wholesale_quantities=' . ( isset( $_COOKIE[ $wholesale_cookie_name ] ) ? intval( !! $_COOKIE[ $wholesale_cookie_name ] ) : 0 );
 ...
 * @param array|string  $roles   Role to store in cookie
 * @param int           $expire  Cookie lifetime
 */
function wpsc_create_role_cookie( array $roles, $expire ) {
	if( wpsc_can_send_auth_cookies() ) {
		sort( $roles );

		setcookie( 'wpsc_role', wpsc_hash_cookie_value( join( '-', $roles ) ), $expire, '/' );
	}
}

/**
 * Disable cache if role cookie does not exists
 */
function wpsc_on_cache_init_maybe_disable_cache_based_on_role_cookie_exiting() {
	$cookiehash = defined( 'COOKIEHASH' ) ? preg_quote( constant( 'COOKIEHASH' ) ) : '';
	$authenticated = false;
	foreach( array_keys( $_COOKIE ) as $key ) {
		if( strpos( $key, ( 'wordpress_logged_in_' . $cookiehash ) ) !== false ) {
			$authenticated = true;
			break;
		}
	}

	if( $authenticated && ! isset( $_COOKIE[ 'wpsc_role' ] ) && ! defined( 'DONOTCACHEPAGE' ) ) {
...
function wpsc_create_role_missed_cookie() {
	if ( is_user_logged_in() ) {
		...
		wpsc_create_role_cookie( (array)$user->roles, time() + DAY_IN_SECONDS );
	}
}
...
function wp_cache_on_auth_cookie_clean() {
	if( wpsc_can_send_auth_cookies() ) {
		setcookie( 'wpsc_role', ' ', time() - YEAR_IN_SECONDS );
	}
}
